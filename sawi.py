...
def get_indonesia_time():
    """Mendapatkan waktu saat ini dalam timezone Indonesia (WIB)"""
...
def halaman_kasir():
    ...
    if st.button("🧾 Cetak Struk"):
        ...
        if not stok_kurang:
            ...
            # UBAH BAGIAN INI - gunakan waktu Indonesia
            now = get_indonesia_time()  # Menggunakan waktu Indonesia
...
def halaman_laporan():
    ...
    if filtered.empty:
        ...
        # Gunakan waktu Indonesia untuk footer
        waktu_cetak = get_indonesia_time().strftime('%d %B %Y %H:%M:%S')
...
# Alternatif: Jika tidak bisa install pytz, gunakan offset manual
def get_indonesia_time_manual():
    """Alternatif tanpa pytz - tambah 7 jam ke UTC"""
    import datetime
    utc_now = datetime.datetime.utcnow()
    # Tambah 7 jam untuk WIB
    indonesia_time = utc_now + datetime.timedelta(hours=7)
